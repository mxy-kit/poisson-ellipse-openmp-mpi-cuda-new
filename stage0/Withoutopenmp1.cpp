...
// Check if point (x, y) is inside the domain x^2 + 4y^2 < 1
bool if_is_in_D(double x, double y) {
    return (x * x + 4.0 * y * y < 1.0);
}

// Compute length of intersection between segment and domain
...
void fic_reg(std::vector<std::vector<double>>& a,
    ...
    for (int i = 1; i <= M; ++i)
        for (int j = 1; j <= N; ++j) {
            ...
            a[i][j] = (std::abs(lij_a - h2) < 1e-9) ? 1.0 : (lij_a < 1e-9 ? 1.0 / eps : (lij_a / h2) + (1.0 - lij_a / h2) / eps);
            b[i][j] = (std::abs(lij_b - h1) < 1e-9) ? 1.0 : (lij_b < 1e-9 ? 1.0 / eps : (lij_b / h1) + (1.0 - lij_b / h1) / eps);
        }
...
double dot(const std::vector<std::vector<double>>& u,
           const std::vector<std::vector<double>>& v, int M, int N, double h1, double h2) {
    double sum = 0.0;
...
// ---------------- MAIN ----------------
int main() {
    std::vector<std::pair<int, int>> grids = {{10, 10}, {20, 20}, {40, 40}};
    double delta = 1e-6;

    for (const auto& grid : grids) {
        int M = grid.first, N = grid.second;
