    ...
    int local_nx, int local_ny)
{
    ...
    //    We need them on the extended range including halos.
 ...
 *  - Within each process, loops are parallelised with OpenMP.
                        ...
                        int rank, int size, MPI_Comm comm)
{
    ...
    for (int k = 1; k <= max_iter; ++k) {
        ...
        #pragma omp parallel for collapse(2) reduction(+:local_diff)
        for (int li = 1; li <= local_nx; ++li) {
            for (int lj = 1; lj <= local_ny; ++lj) {
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        std::cout << "MPI/OpenMP run with " << size << " MPI processes; "
                  << "M=" << M << ", N=" << N << std::endl;
    }
